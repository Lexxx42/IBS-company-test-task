...
def get_received_response_body_for_ui_put_update(name_to_send, job_to_send, user_id=2) -> dict:
    request = PutUpdate('users/', name='', job='')
    request.send_request_with_name_job_and_id(request, name_to_send, job_to_send, user_id)
    return request.get_response()


@allure.step('Get request URL')
def get_request_url_for_ui_put_update(user_id=2) -> str:
    request = PutUpdate('users/', name='', job='')
...
class TestPutUpdate:
    ...
    def test_put_update_should_return_status_code_201(self, name_to_send, job_to_send, user_id):
    ...
    @pytest.mark.parametrize('user_id', range(1, 6))
    @pytest.mark.parametrize('name_to_send, job_to_send', [('', '')] * 2)
    def test_put_update_sent_data_must_be_in_response(self, name_to_send, job_to_send, user_id):
        person = next(generated_person())
        name_to_send = person.name
        job_to_send = person.job
        sent_name, sent_job = \
    ...
    @pytest.mark.parametrize('user_id', range(1, 6))
    @pytest.mark.parametrize('name_to_send, job_to_send', [('', '')] * 2)
    def test_put_update_have_all_data_in_response(self, name_to_send, job_to_send, user_id):
        person = next(generated_person())
        name_to_send = person.name
        job_to_send = person.job
        request.send_request_with_name_job_and_id(request, name_to_send, job_to_send, user_id)
    ...
    def test_put_update_returns_400_when_no_name_provided(self, job_to_send, user_id):
        person = next(generated_person())
        job_to_send = person.job
